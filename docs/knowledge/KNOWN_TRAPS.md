...
## 27. 克隆数字舵机拒绝带外脉冲——"开环账本"被静默脱钩
**现象**: ①命令舵机去逻辑 0° 以外的微调位置，舵机原地不动；②随后发 H，舵机先猛弹到零端再开始归中——两个现象同根。中间角度用 trim 校准正常，唯独贴着行程边界的角度怎么都到不了
**原因**: 该克隆 MG996R（数字版）只接受 500~2500µs 脉宽，出界脉冲**整条忽略并保持原位**（不是 clamp 到边界）。固件侧为补偿舵机盘机械偏差加 `trim=-7` 后，逻辑 0° 需要发 422µs——被忽略。更隐蔽的是开环记账：`M1 0` 把账面 current 置 0，物理舵机没动；H 归中的斜坡前 ~1 秒全在发无效脉冲，舵机装死；斜坡爬回有效带瞬间舵机突然追上——表现为"先弹到零度再归中"
**正确做法**: ①trim 只能在有效脉宽带内平移，越界需求必须用**机械法**消化（松开舵机盘重对花键齿，每齿 ~1.6°，装配日逐轴做）；②固件脉宽计算永远 clamp 回有效带内，宁可到不了也不发无效脉冲（发了会脱钩）；③有符号/无符号混算警示：`(uint32_t)负数` 回绕后经乘除产生看似合法的垃圾脉宽（本案 -7 → 610µs ≈ +10°，方向都反了），角度→脉宽全程用 int32
**发现于**: 2026-08-25 实机验证 trim 时（用户报"点 0 不动、H 先弹跳"，串口+SWD 寄存器级诊断定位）
