...
## 27. 负角度强转 uint32 → CCR 大于 ARR = 恒高电平 = 舵机"失联"保持
**现象**: trim 补偿后命令逻辑 0°，舵机不动；随后发 H，舵机先猛弹到零端再开始归中。中间角度（90/180）trim 校准正常，唯独贴逻辑带边界的角度异常
**原因**: 两层。①直接原因：脉宽计算 `(uint32_t)phys` 把 phys=-7 回绕成 42 亿，乘除回绕后 cmp=24293，而 ARR=19999——**比较值大于周期，PWM 引脚恒为高，无边沿=舵机收不到信号=原地保持**；H 斜坡爬回有效区瞬间脉冲恢复，舵机从老位置猛跳，表现为"弹到零端再归中"。②诊断弯路：曾误判为"舵机拒绝 500µs 以下脉冲"，实机证伪——舵机行程确实越出逻辑带，问题全在固件算术
**正确做法**: ①角度→脉宽全程 int32 有符号运算，禁止把可能为负的物理量强转无符号（回绕+截断双重污染）；②允许 trim 越出逻辑带时（±30° 余量），必须核对 cmp 极值仍落在 (0, ARR) 区间内；③开环系统里"账面位置"与"物理位置"脱钩的表现形式是迟滞/弹跳，看到这类现象优先怀疑执行器根本没收到有效指令
**发现于**: 2026-08-25 实机验证 trim 时（用户两次实测纠错："不是物理极限""发零不动、H 先弹跳"，最终定位 CCR>ARR 溢出）
