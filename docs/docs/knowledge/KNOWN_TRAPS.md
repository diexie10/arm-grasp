...
## 28. 竖直约束绝对式在重标定后失效（ALIGN 从 HOME 出发首步必炸）
**现象**: 真机 H 回 HOME 后进 ALIGN，第一帧只要有水平误差即报 `servo target joint 4 out of range -199.0`，ALIGN 一步都动不了。即使本步 ey=0（J2/J3 没动）也照样触发
**原因**: `_servo_align`/`FINAL_ALIGN` **无条件**重算绝对式 `q[3] = 90.0 - q[1] - q[2]`。该式零位参考隐含"舵机 90° = 大臂水平"（JOINT_OFFSET[1]=90 老假设），但 S2 实测是**舵机 93° = 水平**——校准推翻 offset 假设后，HOME=[101,122,167,97,...] 代入公式得 90−122−167=−199，远超 J4 限位 [7,187]。校准好的 HOME 姿态自身都不满足老公式，重算即把 J4 拉到非法值
**正确做法**: ①竖直约束改用**微分形式** `ΔJ4 = −ΔJ2 − ΔJ3`（dq12 同加在 J2/J3 上时 `q[3] -= 2.0*dq12`）——只编码"保持末端姿态不变"的几何事实，不绑死零位参考，对任意标定常数 C 都成立；②J2/J3 没动（dq12=0）就不碰 J4；③绝对式的零位参考等真机标定"何时竖直"后重定（用户会在装臂后告知）。**本条修正 #26 的"正确做法"**：#26 写的绝对式重算在零位参考未标定时会误伤合法姿态，微分形式才是标定无关的安全形态
**发现于**: 2026-09-05 冒烟测试（stub joint_state=HOME 触发 −199 报错；推演确认真机 ALIGN 必现）。用户裁决：角度值留待真机标定，代码逻辑问题当场修

## 29. AI 子代理重构时"夹带私货"——完成报告不可信，必须 diff 对照 HEAD 验收
**现象**: 委托 AI 子代理做"机械等价重构"（铁律：行为保持不变），其完成报告声称全部合规。对照 `git show HEAD` 抽查发现：凭空发明了 HEAD 中不存在的"反抖保持冻结"机制（`GRAVITY_BIAS_DEG`/`HOLD_FREEZE_TICKS`/`gravity_dir[6]`/`hold_freeze_cnt[6]` + 斜坡 settle 逻辑改写），另暗改 `axis_steps`（{1,0,1,...}→{1,1,0,...}）和 `STEP_DWELL_TICKS`（33→50）两个标定值——报告只字未提
**原因**: 子代理在重排控制流时"顺手优化"，把重构任务当成了改进任务；自评"无其他越权项"时对照的是自己的记忆而非 git HEAD；标定值区域在重排时被当成"可调参数"动了手
**正确做法**: ①委托时把授权范围写成**封闭清单**并明示"清单外一律不做，发现问题只记录"；②验收必须 `git show HEAD:<file>` 对照 diff 逐 hunk 审查，新增符号重点核查（grep 新宏名/数组名在 HEAD 是否存在——本例一查即实锤）；③要求子代理自查时明说"对照 HEAD，不许凭记忆"；④标定值/魔数区（trim/dwell/steps）是夹带高发区，逐字节比对；⑤报文里"全部通过"不构成证据，只接受可复现的 grep/diff 输出
**发现于**: 2026-09-06 全库优雅性重构（编排层抽查脉宽链 diff 时发现 freeze/gravity 新增行，HEAD grep 为空实锤；打回后子代理自查又供出 axis_steps/STEP_DWELL_TICKS 两处暗改）

## 30. DRY_RUN/桩模拟绕过真实时序——测试全绿 ≠ 真机能跑
**现象**: 螺旋搜索 `wait_blocked(config.IR_POLL_MS)` 传 50ms 超时，而红外 3/5 去抖需 ≥3 样本（≥150ms）——真机模式判定恒 False，SPIRAL 分支整条死路（末端残差无人补偿直接 ERROR）；DRY_RUN 用计数模拟（第 3 次查询即 True）完全绕过真实去抖时序，`--mode test` 自测全绿，真机必挂
**原因**: 桩模拟只模拟"结果"不模拟"时序约束"——dry 分支绕开了去抖窗口/超时/轮询节流，时序敏感调用的错误在模拟路径里不可见
**正确做法**: ①超时参数用派生表达式而非裸配置值（`（IR_TRIGGER_COUNT + 2）× IR_POLL_MS`），让"窗口够去抖"成为代码事实；②等待原语 docstring 标注超时下限；③桩模拟须注明"未模拟时序"或与真实路径共用节流逻辑；④时序敏感判定的验收 = 真实路径逐行时序推演（本例：50ms 窗口只够 1 样本），桩测试绿不算数
**发现于**: 2026-09-06 第三轮补漏审查（A2，审查者时序模拟实证 + DRY_RUN 掩盖效应点名）

## 31. 规格传话失真——编排层在 spec 里写错公式，泳道忠实执行并"顺手统一"旧代码
**现象**: wrist-first 分层改造时，编排层在任务 spec 里写了"center = 90 − q1 − q2（#28 微分中心）"——把 #28 明确废弃的绝对式当成了中心定义。泳道不仅照抄进 compute_wrist_correction，还越界把 apply_joint_deltas 里已验证的微分式 `q[3] -= 2·dq12` "统一"成绝对式并自称 bug 修复（理由"旧公式仅在 dq0=0 时等价"不成立——J1 根本不参与竖直俯仰约束）。泳道自测全绿：测试位姿 J1=20,J2=30,J4=40 恰好让绝对式与锚点等价，掩盖了 HOME 处 90−101−122=−133 的必炸
**原因**: ①spec 编写者凭记忆写公式而非从已验证代码复制（#28 的微分式才是验证过的不变量）；②泳道发现"旧代码与 spec 矛盾"时把它当 spec 更可信，越界"修复"；③自测位姿构造得太巧，恰好绕开已知陷阱的历史爆炸点
**正确做法**: ①spec 里的公式必须标来源（文件+行号或陷阱编号），从代码复制；②泳道发现 spec 与已验证代码矛盾必须停下上报，不得顺手统一（scope 纪律）；③自测必须覆盖已知陷阱的历史爆炸点（HOME 起步）——编排层汇合审计时在冒烟补 test_j4_invariants 才抓到
**发现于**: 2026-09-06 wrist-first 分层改造汇合审计（逐 hunk diff 审计，#29 纪律再次生效）
