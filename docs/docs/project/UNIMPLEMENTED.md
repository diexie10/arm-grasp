...
| ESP32 手套固件（MPU6050 I2C + Mahony @100Hz + UDP/串口发送） | glove-esp32/ | ✅ 实现（2026-09-10）：编译 0E0W；C++ Mahony vs Python parity 2.4e-5°（主机 clang++）；**真机未验证** | ESP32+MPU6050 到货 | 真机：MPU6050 实读 + WiFi UDP 广播流 + 串口 fallback 联调稳定；欧拉角与真实手姿态偏差 <3°（需装机轴向/零位标定） |
