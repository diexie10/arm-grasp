...
| 功能 | 位置 | 状态 | 前置依赖 | 验收标准 |
|------|------|------|---------|---------|
| PC 手套桥（UDP/串口/sim 源 → 映射 → 20Hz M 流 → 断流冻结/急停门控） | pc/glove/glove_bridge.py | ✅ 实现（2026-09-10）：DRY_RUN 12s 240/240 发送零回显不符；断流冻结+恢复实测；'e' 急停发 E / 任意发送失败保守中止为代码审查验证（真机按键待验） | 手套方案 v0.2 | 真机 COM8 联调复验 |
| ESP32 手套固件（MPU6050 I2C + Mahony @100Hz + UDP/串口发送 + 开机零偏校准 + 串口 CAL/ZERO） | glove-esp32/ | ✅ 实现（2026-09-10）：编译 0E0W；C++ Mahony vs Python parity 2.4e-5°（主机 clang++）；零偏校准/重零**真机未验证** | ESP32+MPU6050 到货 | 真机：MPU6050 实读 + WiFi UDP 广播流 + 串口 fallback 联调稳定；零偏校准效果与静止度实测；欧拉角与真实手姿态偏差 <3°（需装机轴向/零位标定） |
